...
<header class={`fixed top-0 left-0 right-0 border-b z-50 h-16 shadow-lg transition-colors ${isDarkMode ? 'bg-gray-950 border-gray-800' : 'bg-white border-gray-200'}`}>
    ...
    >
      ...
      <div class="p-8">
        <div class="text-center mb-6">
          <h2 class={`text-2xl font-bold ${isDarkMode ? 'text-gray-100' : 'text-gray-900'}`}>로그아웃</h2>
          <p class={`text-sm mt-1 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>계정에서 로그아웃하시겠습니까?</p>
        </div>
        ...
        <div class="flex justify-end gap-3">
          <button class={`px-4 py-2 rounded transition-colors ${isDarkMode ? 'bg-gray-800 text-gray-200 hover:bg-gray-700' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`} onclick={() => showLogoutConfirm = false}>취소</button>
          <button class={`px-4 py-2 rounded transition-colors ${isDarkMode ? 'bg-red-600 text-white hover:bg-red-700' : 'bg-red-500 text-white hover:bg-red-600'}`} onclick={async () => { await handleLogout(); showLogoutConfirm = false; }}>로그아웃</button>
        </div>
      </div>
    </div>
  </div>
