...
<div class="min-h-screen flex flex-col bg-gray-50 text-gray-900 font-sans">
  ...
  <div class="w-full max-w-md bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden relative p-8 md:p-10">
      ...
      <div class="relative w-full">
        <!-- 시각적 레이어 (클릭 이벤트 없음, 장식용) -->
        <div
          aria-hidden="true"
          class="w-full flex items-center justify-center bg-white border border-gray-300 text-gray-700 font-semibold py-3 rounded-lg {isLoading ? 'opacity-50' : ''}"
        ...
        <!-- 인터랙션 레이어: 실제 Google 버튼이 정확히 위에 올라감 -->
        <!-- 사용자가 직접 Google 버튼을 클릭하는 구조 → Chrome 보안 정책 통과 -->
        <div
          bind:this={googleBtnContainer}
          class="absolute inset-0 overflow-hidden rounded-lg"
          style="opacity: 0.0001; cursor: {isLoading ? 'not-allowed' : 'pointer'}; pointer-events: {isLoading ? 'none' : 'auto'};"
        ></div>
      </div>
  </div>
  </div>
